...
export default class ContextTestingHelper {
  ...
  constructor() {
    const homeSpecFile = new SpecificationFile(
      path.resolve(__dirname, "specification.yml")
    );

    const codeSpecFile = new SpecificationFile(
      path.resolve(__dirname, "specification.yml")
    );
    this._context = {
      current: "home",
  ...
  createDummyContextFile(): void {
    writeFileSync(DEFAULT_CONTEXT_FILE_PATH, JSON.stringify(this._context), {
      encoding: "utf-8",
    });
  }
  ...
  newCommandHelper() {
    return {
      deleteSpecFile: () => {
        const specificationFilePath = path.resolve(
          process.cwd(),
          "specification.yaml"
        );
